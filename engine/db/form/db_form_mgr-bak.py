...
class DbFormMgr(DbBase):
    ...
    # form list
    def get_all_base_form(self, wp_id=0, uc_id=0):
        """
        get all form
        :return:
        """
        ...
        try:
            ...
            fields = 'id,title,fields_num,u_max_id,creator_id,create_time,updated_time,des'
            ...
            for index, field_item in enumerate(form_info['fieldList']):
                if 's' in field_item['id']:
                    ...
                    sql = self.create_select_sql(db_name, 'fieldTable',
                                                 'id,style,label,default_value,placeholder,value_num,value_list,edit,des,create_time,updated_time',
                                                 condition=condition)
    ...
    def get_field_template(self, style=0, wp_id=0):
        ...
        try:
            ...
            db_name = configuration.get_database_name()
            sql = self.create_select_sql(db_name, 'fieldTable',
                                         'id,u_id,style,label,default_value,placeholder,value_num,value_list,edit,des,create_time,updated_time',
                                         condition=condition)
            # print(sql)
    ...
    def delete_form(self, form):
        ...
        try:
            ...
            if data['code'] == 200:
                condition = "id=%s" % id
                delete_table_sql = self.create_delete_sql(db_name, "formTable", condition)
    ...
    def add_new_form(self, form, workspace_id=0):
        ...
        try:
            ...
            # insert form
            fields = (
            'workspace_id', 'usecase_id', 'title', 'fields_num', 'u_max_id', 'creator_id', 'fields_list', 'des',
            'create_time', 'updated_time')
            values = (
            workspace_id, usecase_id, form_title, fields_num, u_max_id, creator_id, fields_list, des, create_time,
            update_time)
    ...
    def update_form(self, form, account_id, workspace_id):
        ...
        try:
            ...
            # workflow_data = workflow_mgr.get_all_details_workflow_by_form_id(form_id)
            # if workflow_data['code'] == 200:
            #     column = ('available',)
            #     values = (0,)
            ...
            # hsa_workflow_flag = False
            # # get workflow field id
            # workflow_using_fields = set()
            # if workflow_data['code'] == 200 and len(workflow_data['data']) > 0:
            #     hsa_workflow_flag = True
            #     for workflow in workflow_data['data']:
            #         workflow_using_fields = workflow_using_fields | set(workflow['field_id_list'])
            # # form exists
            # # print('workflow_using_fields: ', workflow_using_fields)
            # if select_form['code'] == 200:
            #     # get all old field list
            #     old_filed_set = set()
            #     if hsa_workflow_flag:
            #         for field in select_form['data']['fieldList']:
            #             old_filed_set.add(field['id'])
            #     # print('hsa_workflow_flag:', hsa_workflow_flag)
            #     # build form update sql
            #     form_condition = "id='%s'" % form_id
            #     form_title = form['title']
            #     fields_num = len(form['fieldList'])
            #     u_max_id = 0
            ...
            #                 u_max_num = tp_max_id
            #                 u_max_id = field_id
            #         # check the missing uid
            #         if hsa_workflow_flag and field_id in old_filed_set:
            #             old_filed_set.remove(field_id)
            #     # check if the field is using in workflow
            #     if hsa_workflow_flag:
            #         for deleting_field_id in old_filed_set:
            #             if deleting_field_id in workflow_using_fields:
            #                 data = response_code.UPDATE_DATA_FAIL
            #                 return data
            # update form fields
            fields_list = json.dumps(form['fieldList'])
            ...
            # update form
            fields = ('title', 'fields_num', 'u_max_id', 'creator_id', 'fields_list', 'des', 'updated_time')
            values = (form_title, fields_num, u_max_id, creator_id, fields_list, des, update_time)
