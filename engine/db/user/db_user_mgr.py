...
class DbUserMgr(DbBase):
    ...
    *第一次用给定的admin账号和密码登录；
    ...
    def get_user_by_name(self, name, ldap_username=None, ad_group_list=None):
        """
        ...
        try:
            ...
            if (not user_info) and ldap_username:
                import datetime
                if not ad_group_list:
                    ad_group_list = []
                now = str(datetime.datetime.today())
                user_info = {'ACCOUNT_NAME': ldap_username, 'ACCOUNT_ID': name, 'PASS_WORD': 'ldap_login',
