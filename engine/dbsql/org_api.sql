...
CREATE TABLE `workspaceTable` (
  ...
  `REGIONS` longtext COMMENT 'Need workspace Head approval',
  `CREATE_TIME` datetime DEFAULT NULL COMMENT 'create_time',
  `DES` text DEFAULT NULL COMMENT 'comment',
  PRIMARY KEY (`ID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=351 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
...
CREATE TABLE `usecaseTable` (
  ...
  `RESOURCES_ACCESS_LIST` longtext COMMENT 'usecase_name resource access list',
  `SERVICE_ACCOUNT` longtext COMMENT 'usecase_name service account',
  `CROSS_REGION` longtext COMMENT 'will the usecase_name use data cross region',
  `CREATE_TIME` datetime DEFAULT NULL COMMENT 'create_time',
  `DES` text DEFAULT NULL COMMENT 'comment',
  PRIMARY KEY (`ID`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=351 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
